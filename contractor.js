...
function loadBills() {
  ...
  if (window.db) {
    window.db.collection('bills').where('contractorId', '==', contractorId)
      .onSnapshot(snap => {
        const fb = snap.docs.map(d => d.data());
        if (fb.length > 0) {
          bills = fb;
          // Sort by submission date
          bills.sort((a, b) => new Date(a.submittedAt) - new Date(b.submittedAt));
          saveBills();
          renderStats();
          renderProjects();
          renderBills();
      ...
      }, err => console.warn('Bills live sync error:', err));
  }
}
...
function renderProjects() {
  ...
  const myProjects = projects.filter(p => p.contractorId === contractorId || contractorId === 'CNT001');
...
function openBillModal() {
  ...
  const myProjects = projects.filter(p => p.status === 'Active' || p.contractorId === contractorId);
